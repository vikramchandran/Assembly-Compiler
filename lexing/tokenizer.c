...
size_t SelectToken(char* buffer,
  ...
  */
  ...
  if (buffer[size_read] == '+') {  // + and ++
    ...
    t->linenum = *linenum;
    size_read++;
  } else if (IS_COMMENT || buffer[size_read] == '/') {  // / and comments
    if (size_read + 1 == size) {
      return size_read;
    }
    if (IS_COMMENT || buffer[size_read + 1] == '/') {
      /* YOUR CODE HERE*/
      IS_COMMENT = 1;
      while(IS_COMMENT) {
        if (size_read + 1 < size) {
    ...
    // if(buffer[size_read + 1] == '\\') {
    //   size_read++;
    //   if(buffer[size_read + 1] == 'n') {
    //     (*linenum)++;
    //     size_read++;
    //   }
    //   else if(buffer[size_read + 1] == 't') {
    //     size_read += 
    //   }
    //   else if(buffer[size_read + 1] == 'r') {

    //   }
    //   else if(buffer[size_read + 1] == 'v') {
        
    //   }
    //   else if(buffer[size_read + 1] == 'f') {
        
    //   }
    // }

    if ((size_read + 3 < size) && (replace_escape_in_character(buffer + size_read + 1) != -1)) {
      if (buffer[size_read + 3] == '\'') {
        t = create_token(filename);
        t->linenum = *linenum;
    ...
    // Now check if value after escape is an end quote, otherwise error
    // Now check if value after regular character is an end quote, otherwise moving onto next

    /* YOUR CODE HERE */
    else if ((size_read + 2 < size) && isprint(buffer[size_read + 1]) && buffer[size_read + 2] == '\'') {
      t = create_token(filename);
      t->linenum = *linenum;
      t->type = TOKEN_CHARACTER;
    ...
    while (size_read + int_len < size && search) {
      if (is_digit(buffer[size_read + int_len])) {
        ...
        // t->data.integer = buffer[size_read];
        else  {
          t = create_token(filename);
          t->linenum = *linenum;
          t->type = TOKEN_INTEGER;
          char * c = (char *)malloc((int_len * sizeof(char)) + 1);
          int count = int_len;
          for(int i = 0; i < count; i++) {
            c[i] = buffer[size_read + i];
          }
          c[int_len] = '\0';
          t->data.integer = strtol(c,NULL, 0);
          free(c);
          size_read += int_len;
        }
    ...
    while (size_read + id_len < size && search) {
      if (is_identifier_component(buffer[size_read + id_len])) {
        ...
        if (type != TOKEN_ERR) {
          ...
          char * strr = (char *) malloc((id_len * sizeof(char)) + 1);
          for(int i = 0; i < id_len + 1; i++) {
            strr[i] = token_contents[i];
          }
          t = create_token(filename);
          t->linenum = *linenum;
          t->type = TOKEN_IDENTIFIER;
          t->data.identifier = strr;
          size_read += id_len;
