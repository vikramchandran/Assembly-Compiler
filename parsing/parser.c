...
AST* BinaryExpr(TokenList** tokens,
  ...
  while(shouldcontinue != 0 && validator == 1) {
    shouldcontinue = 0;
    for (int i = 0; i < size; i++) {
      if (ProcessToken(tokens, possible_tokens[i])) {
        shouldcontinue = 1;
        AST* newast = MakeAST(ast_choices[i], (*tokens)->t->filename,
                           (*tokens)->t->linenum);
        AppendAST(newast, first_ast);
        AppendAST(newast, GetNextBinaryExpr(tokens, type));
        first_ast = newast;
      } 
    }
}
return first_ast;
  //Loop that iterates through the tokens
  //What currently works is just two operator operations, having more than two operations won't work.
  //We have to make it work such that there are more than two operations that work at a time
  //Having the same operator unforuantely won't work though
}
...
AST* GetNextBinaryExpr(TokenList** tokens, enum BinaryExprType type) {
  ...
  } else if(type == BINARY_EXPR_MULTDIV) {
    return PrefixExpr(tokens);
  }
  return BaseExpr(tokens);
  //Change this BaseExpr to something
}
